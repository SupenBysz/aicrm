...
仓库同时维护两层范围：Phase 1 是已经锁定的多租户身份、组织、权限和 AI 配置基线；Post-Phase1 v9.1 正在建设 Matrix Account、可信 Desktop、Agent Executor、Codex 授权和 AI 登录脚本维护闭环。v9.1 已完成 P0 合同覆盖与安全止血，当前进入 P1 additive schema、新服务骨架和 shadow read；新增账号自动化、可信快照完成和 AI 自动修复的生产门禁仍全部关闭。
...
Phase 1 基线不包含、但部分已进入 Post-Phase1 v9.1 受控建设的能力：

- 客户、线索、商机、合同等 CRM 业务模块。
- AI 员工、通用 AI 工作流和 AI 协作仍不在范围；Agent Executor 只为 v9.1 Codex 授权、模型目录、任务执行和脚本维护服务。
- IM、移动端应用。
...
- Post-Phase1 v9.1 执行架构：`docs/kyai_crm_v9_execution_architecture.md`
- Matrix Account v9：`docs/kyai_crm_matrix_account_requirements.md`
- Agent Executor 授权 v9：`docs/kyai_crm_ai_executor_authorization_requirements.md`
- 新增账号编排合同：`docs/matrix_account_ai_onboarding_contract.md`
