...
func (s *Store) ListAuditLogs(ctx context.Context, wsType, wsID string, platformGlobal bool, action, resourceType, actorUserID, startAt, endAt string, actorMembershipIDs []string, page, pageSize int) ([]AuditLog, int64, error) {
	...
	if !platformGlobal {
		where = append(where, "workspace_type="+add(wsType))
		where = append(where, "workspace_id="+add(wsID))
	}
	if action != "" {
		where = append(where, "action="+add(action))
	}
	if resourceType != "" {
		where = append(where, "resource_type="+add(resourceType))
	}
	if actorUserID != "" {
		where = append(where, "actor_user_id="+add(actorUserID))
	}
	if startAt != "" {
	...
	if actorMembershipIDs != nil {
		if len(actorMembershipIDs) == 0 {
			...
			args = append(args, a...)
			where = append(where, "actor_membership_id IN ("+ph+")")
		}
	}
	clause := "TRUE"
	if len(where) > 0 {
	...
	var total int64
	if err := s.db.QueryRowContext(ctx, `SELECT count(*) FROM ky_audit_log WHERE `+clause, args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	limit := add(pageSize)
	offset := add((page - 1) * pageSize)
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, actor_user_id, actor_membership_id, workspace_type, workspace_id, action, resource_type, resource_id, result, request_id, source, remark, created_at
		FROM ky_audit_log WHERE `+clause+` ORDER BY created_at DESC LIMIT `+limit+` OFFSET `+offset, args...)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var a AuditLog
		if err := rows.Scan(&a.ID, &a.ActorUserID, &a.ActorMembershipID, &a.WorkspaceType, &a.WorkspaceID, &a.Action, &a.ResourceType, &a.ResourceID, &a.Result, &a.RequestID, &a.Source, &a.Remark, &a.CreatedAt); err != nil {
			return nil, 0, err
		}
