...
- `MALS-V6-01-05` legacy Desktop Bridge v1 仅在显式声明 `supportsDeferredWindowRelease=true` 时启用自动识别；检测阶段保留受控窗口，绑定成功或结果未知时只关闭窗口、不清理 WebSpace，已知绑定失败后才允许清理。

验收：

- 用户扫码后无需手动点击识别。
- 识别阶段不会重新创建 Web 空间，也不会释放当前 Profile。
- 绑定请求进行中关闭侧滑时必须等待结果收敛，禁止并发执行 bind 与 clear。
