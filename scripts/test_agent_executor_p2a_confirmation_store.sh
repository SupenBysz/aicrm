#!/usr/bin/env bash
# Destructive P2A operation-confirmation store test against disposable PostgreSQL.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ADMIN_DB="${KY_AGENT_EXECUTOR_TEST_ADMIN_DATABASE:-postgres}"
RUN_ID="$(date +%s)_$$"
TEST_DB="aicrm_agent_confirmation_${RUN_ID}"
LOGIN_ROLE="ky_agent_confirmation_test_${RUN_ID}"
LOGIN_PASSWORD="ConfirmationTest${RUN_ID}Only"
...
assert_scalar "SELECT (has_table_privilege('ky_agent_executor_writer','ky_ai_executor_operation_confirmation','INSERT') AND has_table_privilege('ky_agent_executor_writer','ky_ai_executor_operation_confirmation','UPDATE'))::int" "1" "writer owns confirmation state"
assert_scalar "SELECT has_table_privilege('ky_agent_executor_writer','ky_ai_executor_operation_confirmation_audit','INSERT')::int" "1" "writer can append confirmation audit"
assert_scalar "SELECT has_table_privilege('ky_agent_executor_writer','ky_ai_executor_operation_confirmation_audit','UPDATE')::int" "0" "writer cannot mutate confirmation audit"
assert_scalar "SELECT has_table_privilege('ky_agent_executor_writer','ky_audit_log','INSERT')::int" "0" "writer cannot mutate cross-service audit log"
...
(cd "$ROOT_DIR/services/ky-agent-executor-service" && \
  GOWORK=off GOFLAGS=-mod=readonly KY_AGENT_EXECUTOR_CONFIRMATION_TEST_DATABASE_URL="$CONTROL_URL" \
  go test -race -run '^TestOperationConfirmationAgainstPostgres$' -v ./internal/store)

echo 'Agent Executor P2A operation confirmation store contract passed'
